...
```
用户 ←→ OpenClaw Agent ←→ ClawMeeting 插件  (本项目)
                                │
...
1. 将本目录放置到 `D:\lll\mt\clawmeeting`（或任意 OpenClaw 可发现的路径）
...
4. 验证: `openclaw plugins inspect clawmeeting`
...
- 持久化到 `~/.openclaw/clawmeeting/session.json`，OpenClaw 重启后自动恢复
- 轮询推送时指定该 session，避免创建新对话窗口
- **纯插件端机制，不影响服务端 API 接口格式**

## 本地存储

存储目录：`~/.openclaw/clawmeeting/`
...
输出示例：
```
=== ClawMeeting Meeting Negotiator ===
